...
public class HttpUploadServerHandler extends SimpleChannelInboundHandler<HttpObject> {
    ...
    private HttpFileTransportProcess transportProcess;
    ...
    public HttpUploadServerHandler() {
        this.transportProcess = new HttpFileTransportProcess();
    }

    public HttpUploadServerHandler(HttpFileTransportProcess transportProcess) {
    ...
    public void channelRead0(ChannelHandlerContext ctx, HttpObject msg) throws Exception {
        if (msg instanceof HttpRequest) {
            ...
            try {
                transportProcess.beforeTransport(request);
            } catch (Exception ex) {
                responseContent.setLength(0);
                responseContent.append(ex);
                writeResponse(ctx.channel(), HttpResponseStatus.BAD_REQUEST,false);
                throw ex;
            }
        ...
        if (decoder != null) {
            if (msg instanceof HttpContent) {
                ...
                try {
                    ...
                    e1.printStackTrace();
                    responseContent.append(e1.getMessage());
                    writeResponse(ctx.channel(), HttpResponseStatus.BAD_REQUEST, true);
                    return;
                }
    ...
    private void readHttpDataChunkByChunk(Channel channel) {
        try {
                    ...
                    if (partialContent == data) {
                        ...
                        if (data.getHttpDataType() == HttpDataType.FileUpload) {
                            ...
                            if (fileUpload.isCompleted()) {
                                try {
                                    transportProcess.afterTransport(new FileUploadWrap(fileUpload, attributeMap));
                                } catch (Exception ex) {
                                    responseContent.setLength(0);
                                    responseContent.append(ex);
                                    writeResponse(channel, HttpResponseStatus.BAD_REQUEST,false);
                                    throw ex;
                                }
                            }
                        }
            ...
            if (data != null) {
                ...
                //transportProcess.afterTransport("end");
            }
    ...
    private void writeResponse(Channel channel) {
        writeResponse(channel, HttpResponseStatus.OK, false);
    }

    private void writeResponse(Channel channel, boolean forceClose) {
        writeResponse(channel, HttpResponseStatus.OK, forceClose);
    }

    private void writeResponse(Channel channel, HttpResponseStatus status, boolean forceClose) {
    ...
    public static class FileUploadWrap {
        private Map<String, String> attributeMap = new HashMap<>();
        private FileUpload fileUpload;

        public FileUploadWrap(FileUpload fileUpload, Map<String, String> attributeMap) {
