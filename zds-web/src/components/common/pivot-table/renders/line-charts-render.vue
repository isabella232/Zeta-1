...
<script lang='ts'>
	...
	@Component
export default class LineChartRender extends Vue implements PivotTableRender {
  ...
  renderChart() {
    ...
    if (this.chartInstance) {
      ...
      this.chartInstance.setOption(
        this.getLineChartsOptions(this.rowsGenerator(), this.colKeys)
      );
      this.$nextTick().then(() => {
        this.chartInstance.resize();
        this.loading = false;
  ...
  onPlotDataChange(newVal: PivotData, oldVal: PivotData) {
    if (!_.isEqual(newVal, oldVal)) {
      console.debug('pivot data change', newVal, oldVal);
      this.renderChart();
    }
  }
}
