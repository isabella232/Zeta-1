    ...
    >
      <div class="tint fluid-row">
        ...
        >
          ...
          <template>
            ...
            <el-button
              ...
              @click="onStop"
            >
              <!-- :class='{ disable: !canStop(notebook) }' -->
              <!-- <i class="zeta-icon-stop"/> -->
              Stop
              <i
                v-if="stopping"
                class="el-icon-loading"
        ...
        <div class="right">
          ...
          />
          <used-capacity
            v-if="!viewOnly && currentCodeTypeIsSql"
            slot="reference"
            ref="capacity"
          ...
          <!-- <favorite-icon
            ...
            v-click-metric:NB_TOOLBAR_CLICK="{name: 'favorite'}"
          /> -->
          <ShareLink
            v-model="shareUrl"
            title="Share"
...
export default class EditorWrapper extends Vue {
  ...
  reload() {
    if (!this.notebook.loaded) {
      ...
      // let {
      //     data: file
      // }: { data: RestPacket.File } =  this.notebookRemoteService.getById(
...
.control.readonly + .editor {
    height: calc(100% - #{$control-height + 27px});
}
