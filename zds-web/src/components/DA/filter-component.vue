...
export default class FilterComponent extends Vue {
  ...
  get filteredOptions() {
    ...
    return this.optionsData.filter((data) => {
      if (typeof data === 'string') {
        return matchString(searchTxt, data);
      } else {
        return matchString(searchTxt, data.key) || matchString(searchTxt, data.label);
      }
    });
  }
  ...
  formatPltfrm(val: any) {
    return _.upperFirst(_.toLower(val).replace('numozart', 'mozart'));
  }
