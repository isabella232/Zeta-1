...
import UpperSQLReserved from "../core/UpperSQLReserved";
...
const reservedWords = [
    ...
    "COMMENT", "COMMIT", "COMMITTED", "COMPRESSED", "CONCURRENT", "CONSTRAINT", "CONTAINS", "CONVERT", "CROSS",
    "CURRENT_TIMESTAMP",
...
const reservedNewlineWords = [
    "AND",
    "CREATE", "CROSS APPLY", "CROSS JOIN",
    "ELSE",
    "GROUP BY",
    "INNER JOIN", "INSERT",
    "JOIN",
    "LEFT JOIN", "LEFT OUTER JOIN", "FULL OUTER JOIN",
    "ORDER BY", "OUTER APPLY", "OUTER JOIN",
...
export default class SparkSqlFormatter {
    ...
    format(query) {
        if (!tokenizer) {
            tokenizer = new Tokenizer({
                ...
                /**
                 * comments by tianrsun @2019-04-12
                 * + string type: `${}`
                 */
                stringTypes: [`""`, "N''", "''", "``", "[]", '${}'],
                openParens: ["(", "CASE"],
    ...
    UpperSQLReserved(query) {
        ...
        return new UpperSQLReserved(this.cfg, tokenizer).format(query);
    }
}
